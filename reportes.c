...
//Arreglar los parametros que pide y recibe.
void historialCliente(Reserva reservas[], Cliente clientes[], int cantReservas, int cantClientes, int idCliente) {
    ...
    printf("El historial de reservas del cliente: %s, con ID: %d", nombre, idCliente);
    for (int i = 0 ; i < cantidadReservas; i++) {  //Recorremos la cantidad de reservas
        if(reservas[i].idCliente == idCliente){  //Comparamos si el id de la reserva coincide con el id del cliente
            ...
            printf("Reserva: %d, funcion %d, cantidad de entradas %d", reservas[i].id, reservas[i].idFuncion, reservas[i].cantidad); //Imprimimos los datos de la reserva encontrada
...
//Testear
void reportePromedio(Sala salas[], int cantidadSalas, Funcion funciones[], int cantFunciones) {
